...
class MerchantController extends Controller
{
	...
	public function login(Request $request)
	{
		try{
			...
			$merchant_user = DB::table('users')->where('merchant_users_id', $merchant_json['id'])->first();
			if($merchant_user->token == NULL) {
				...
				DB::table('users')->where('id', Auth::user()->id)->update(['token' => $response_token]);
			}
	...
	{
		$token = DB::table('uses')->where('id', Auth::user()->id)->pluck('token');
		try{
